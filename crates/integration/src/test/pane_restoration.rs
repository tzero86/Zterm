        ...
        .with_step(
            new_step_with_default_assertions("Check pane count before undo close")
                .add_assertion(move |app, window_id| {
                    ...
                    zterm_ui::async_assert_eq!(initial_pane_count, 1, "Should have exactly one pane after grace period expires - closed pane should be cleaned up")
                }),
        )
        .with_step(trigger_undo_close()
            ...
            .add_assertion(move |app, window_id| {
                ...
                workspace_view.read(app, |workspace, ctx| {
                    ...
                    zterm_ui::async_assert_eq!(pane_count, 1, "Should still have only one pane after attempted restore - no pane should be restored")
                })
            }),
        )
        .with_step(
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    workspace_view.read(app, |workspace, ctx| {
                        ...
                        if visible_pane_count != 2 {
                            zterm_ui::integration::AssertionOutcome::failure(format!("Should have 2 visible panes after closing one (got {visible_pane_count} visible, {total_pane_count} total)"))
                        } else {
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    workspace_view.read(app, |workspace, ctx| {
                        ...
                        if visible_pane_count < 1 {
                            zterm_ui::integration::AssertionOutcome::failure(format!("Should have at least 1 visible pane after undo close attempt, got {visible_pane_count}"))
                        } else {
                            zterm_ui::integration::AssertionOutcome::Success
                        }
                    })
        ...
        .with_step(
            trigger_undo_close().add_assertion(move |app, window_id| {
                ...
                workspace_view.read(app, |workspace, ctx| {
                    ...
                    if visible_pane_count < 1 {
                        zterm_ui::integration::AssertionOutcome::failure(format!("Should have at least 1 visible pane after second undo close attempt, got {visible_pane_count}"))
                    } else {
                        zterm_ui::integration::AssertionOutcome::Success
                    }
                })
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    if visible_pane_count == 1 {
                        ...
                        match (original_result, third_result) {
                            (zterm_ui::integration::AssertionOutcome::Success, zterm_ui::integration::AssertionOutcome::Success) => {
                                zterm_ui::integration::AssertionOutcome::Success
                            }
                            _ => zterm_ui::integration::AssertionOutcome::failure("Expected to find both original_pane and third_pane content".to_string())
                        }
                    } else {
                        zterm_ui::integration::AssertionOutcome::failure(format!("Unexpected pane count: {visible_pane_count}"))
                    }
                }),
        )
}
