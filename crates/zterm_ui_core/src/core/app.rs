...
pub struct AppContext {
    ...
    /// that `zterm_ui_core` does not depend on `reqwest`.
    #[allow(clippy::type_complexity)]
...
impl AppContext {
    ...
    /// Used to load fallback fonts without pulling `reqwest` into `zterm_ui_core`.
    ...
    pub fn open_view_tree_debug_window(&mut self, target_window_id: WindowId) {
        ...
        let options = AddWindowOptions {
            ...
            window_instance: Some("dev.warp.zterm_ui-debug".to_owned()),
