...
use zterm_graphql::{object_permissions::AccessLevel, scalars::time::ServerTimestamp};
use zterm_ui_core::{
    Element,
    elements::{
...
impl TryFrom<zterm_graphql::object::ObjectType> for ObjectIdType {
    type Error = anyhow::Error;
    fn try_from(object_type: zterm_graphql::object::ObjectType) -> Result<Self, Self::Error> {
        match object_type {
            zterm_graphql::object::ObjectType::AIConversation => Err(anyhow!(
                "AIConversation is not a supported object type for this operation"
            )),
            zterm_graphql::object::ObjectType::Notebook => Ok(ObjectIdType::Notebook),
            zterm_graphql::object::ObjectType::Workflow => Ok(ObjectIdType::Workflow),
            zterm_graphql::object::ObjectType::Folder => Ok(ObjectIdType::Folder),
            zterm_graphql::object::ObjectType::GenericStringObject => {
                Ok(ObjectIdType::GenericStringObject)
            }
            zterm_graphql::object::ObjectType::Unknown => {
...
impl From<ObjectType> for zterm_graphql::object::ObjectType {
    fn from(value: ObjectType) -> Self {
        match value {
            ObjectType::Notebook => zterm_graphql::object::ObjectType::Notebook,
            ObjectType::Workflow => zterm_graphql::object::ObjectType::Workflow,
            ObjectType::Folder => zterm_graphql::object::ObjectType::Folder,
            ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                JsonObjectType::EnvVarCollection,
            )) => zterm_graphql::object::ObjectType::GenericStringObject,
...
impl CloudObjectStatuses {
    ...
    pub fn render_icon(
        ...
        let icon_and_tooltip_text = if should_show_local_only_indicator {
            Some((
                Icon::Laptop.to_zterm_ui_icon(theme.main_text_color(theme.surface_1())),
                SYNC_STATUS_TOOLTIP_LOCAL_ONLY,
            ))
        } else if should_show_syncing_indicator {
            Some((
                Icon::Refresh.to_zterm_ui_icon(theme.sub_text_color(theme.surface_2())),
                SYNC_STATUS_TOOLTIP_INFLIGHT,
            ))
        } else if should_show_error_indicator {
            Some((
                Icon::AlertTriangle.to_zterm_ui_icon(Fill::Solid(theme.ui_error_color())),
...
impl From<GenericStringObjectFormat>
    for zterm_graphql::generic_string_object::GenericStringObjectFormat
{
    fn from(format: GenericStringObjectFormat) -> Self {
        use zterm_graphql::generic_string_object::GenericStringObjectFormat as GraphQLFormat;
        match format {
...
impl From<CloudObjectEventEntrypoint> for zterm_graphql::object::CloudObjectEventEntrypoint {
    fn from(entrypoint: CloudObjectEventEntrypoint) -> Self {
        use zterm_graphql::object::CloudObjectEventEntrypoint as GraphQLEntrypoint;
        match entrypoint {
...
impl TryFrom<zterm_graphql::object::ObjectMetadata> for ServerMetadata {
    type Error = anyhow::Error;

    fn try_from(value: zterm_graphql::object::ObjectMetadata) -> Result<Self, Self::Error> {
        let folder_id: Option<FolderId> = match value.parent {
            zterm_graphql::object::Container::FolderContainer(folder_container) => {
...
impl TryFrom<zterm_graphql::object_permissions::ObjectPermissions> for ServerPermissions {
    type Error = anyhow::Error;

    fn try_from(
        value: zterm_graphql::object_permissions::ObjectPermissions,
...
impl TryFrom<zterm_graphql::object_permissions::GuestSubject> for ServerGuestSubject {
    type Error = anyhow::Error;

    fn try_from(
        value: zterm_graphql::object_permissions::GuestSubject,
    ) -> Result<Self, Self::Error> {
        match value {
            zterm_graphql::object_permissions::GuestSubject::UserGuest(user_guest) => {
                let guest_subject = ServerGuestSubject::User {
                    firebase_uid: user_guest.firebase_uid.into_inner(),
                };
                Ok(guest_subject)
            }
            zterm_graphql::object_permissions::GuestSubject::PendingUserGuest(guest) => {
                Ok(ServerGuestSubject::PendingUser { email: guest.email })
            }
            zterm_graphql::object_permissions::GuestSubject::TeamGuest(team_guest) => {
                Ok(ServerGuestSubject::Team {
                    team_uid: ServerId::from_string_lossy(team_guest.uid.inner()),
                })
            }
            zterm_graphql::object_permissions::GuestSubject::Unknown => {
...
impl TryFrom<zterm_graphql::object::Container> for ServerObjectContainer {
    type Error = anyhow::Error;

    fn try_from(value: zterm_graphql::object::Container) -> Result<Self, Self::Error> {
        match value {
            zterm_graphql::object::Container::FolderContainer(folder) => {
                Ok(ServerObjectContainer::Folder {
                    folder_uid: ServerId::from_string_lossy(folder.folder_uid.inner()),
                })
            }
            zterm_graphql::object::Container::Space(space) => Ok(ServerObjectContainer::Drive {
                owner: space.try_into()?,
            }),
            zterm_graphql::object::Container::Unknown => {
...
impl TryFrom<zterm_graphql::object::Space> for Owner {
    type Error = anyhow::Error;

    fn try_from(value: zterm_graphql::object::Space) -> Result<Self, Self::Error> {
        let owner = match value.type_ {
            zterm_graphql::object::SpaceType::Team => Owner::Team {
                team_uid: ServerId::from_string_lossy(value.uid.inner()),
            },
            zterm_graphql::object::SpaceType::User => Owner::User {
...
impl From<Owner> for zterm_graphql::object_permissions::Owner {
    fn from(owner: Owner) -> Self {
        use zterm_graphql::object_permissions::Owner as GraphQLOwner;
