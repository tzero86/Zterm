...
/// This purposefully shadows the `Action` trait from `zterm_ui`.
///
/// Since `zterm_ui` depends on this crate, we can't depend on the `zterm_ui::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `zterm_ui::Action` implementing this `Action`.
