...
use zterm_ui::{platform, platform::WindowBounds, AssetProvider};
...
fn main() -> Result<()> {
    ...
    let _ = app_builder.run(move |ctx| {
        root_view::init(ctx);
        let window_options = zterm_ui::AddWindowOptions {
            window_bounds: WindowBounds::ExactSize(vec2f(1000.0, 800.0)),
            window_style: if capture_config.capture_screenshots {
                zterm_ui::platform::WindowStyle::NotStealFocus
            } else {
                zterm_ui::platform::WindowStyle::Normal
            },
            ..Default::default()
        };
