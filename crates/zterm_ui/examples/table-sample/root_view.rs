...
use zterm_ui::platform::CapturedFrame;
...
impl RootView {
    pub fn new(ctx: &mut ViewContext<Self>, capture_config: CaptureConfig) -> Self {
        ...
        let sub_view = ctx.add_typed_action_view(move |ctx| {
            let font_family = zterm_ui::fonts::Cache::handle(ctx)
    ...
    fn render(&self, _app: &AppContext) -> Box<dyn Element> {
        ...
        Container::new(
            Flex::column()
                ...
                .with_cross_axis_alignment(zterm_ui::elements::CrossAxisAlignment::Stretch)
                .with_child(
                ...
                if self.capture_config.capture_screenshots {
                    ctx.spawn(
                        async {
                            ...
                            zterm_ui::r#async::Timer::after(std::time::Duration::from_millis(300))
                                .await;
                        },
                        |view, _, ctx| {
                            view.capture_current_demo(ctx);
