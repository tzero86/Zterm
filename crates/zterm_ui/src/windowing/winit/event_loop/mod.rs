...
/// winit events into zterm_ui events.
#[derive(Default)]
struct State {
...
/// The purpose is cleared later in `handle_converted_zterm_ui_event` after soft keyboard
...
fn convert_touch_ended(
    ...
    // in handle_converted_zterm_ui_event after checking if we need to
    ...
    // handle_converted_zterm_ui_event after soft keyboard logic runs.
...
impl EventLoop {
    ...
    pub fn handle_event(&mut self, evt: Event<CustomEvent>, window_target: &ActiveEventLoop) {
        ...
        match evt {
            ...
            Event::UserEvent(CustomEvent::MomentumScroll { window_id }) => {
                ...
                self.handle_converted_zterm_ui_event(
                    window_id,
    ...
    fn handle_window_event(&mut self, window_id: winit::window::WindowId, evt: WindowEvent) {
        ...
        match event {
            ConvertedEvent::Event(event) => {
                self.handle_converted_zterm_ui_event(window_id, event);
            }
    ...
    fn handle_debounced_drag_drop(&mut self, window_id: winit::window::WindowId) {
        ...
        self.handle_converted_zterm_ui_event(window_id, drag_drop_event);
    }

    /// Handles a request to close the window with the given zterm_ui and winit
    /// IDs.
    ...
    /// Handle events that may be handled by zterm_ui, or maybe not in some cases, e.g. window
    /// drag-to-resize or drag-to-move.
    fn handle_converted_zterm_ui_event(
        &mut self,
        window_id: winit::window::WindowId,
        event: crate::Event,
    ) {
        ...
        // passing the event into zterm_ui. Skip for touch events as drag_resize_window
        ...
        // If the app didn't handle the event, zterm_ui might still want to do something
    ...
    ///
    /// Converts `SoftKeyboardInput` events into zterm_ui `Event`s and dispatches
