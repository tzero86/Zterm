...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if zterm_core::errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "zterm_ui::core::redraw_frame" => sentry_log::LogFilter::Ignore,
...
fn init_log_directory() -> Result<std::path::PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            Ok(zterm_core::paths::state_dir())
        } else if #[cfg(windows)] {
            Ok(zterm_core::paths::state_dir().join(zterm_core::paths::WARP_LOGS_DIR))
        } else {
