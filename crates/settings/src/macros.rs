...
macro_rules! define_settings_group {
    ($group:ident, settings: [$($var:ident: $setting:ident $({ type: $value_type:ty, default: $default:expr, supported_platforms: $supported_platforms:expr, sync_to_cloud: $sync_to_cloud:expr, private: $private:expr $(, storage_key: $storage_key:literal)? $(, toml_path: $toml_path:literal)? $(, max_table_depth: $mtd:literal)? $(, description: $desc:literal)? $(, feature_flag: $flag:path)? $(,)? })? $(,)? )*]) => {
        ...
        impl $group {
            #[allow(dead_code)]
            fn new_from_storage(ctx: &mut zterm_ui::ModelContext<Self>) -> Self {
                use $crate::Setting;
                Self {
                    $(
            ...
            #[allow(dead_code)]
            pub fn new_with_defaults(_ctx: &mut zterm_ui::ModelContext<Self>) -> Self {
                use $crate::Setting;
                Self {
                    $(
            ...
            #[allow(dead_code)]
            pub fn register(ctx: &mut (impl zterm_ui::GetSingletonModelHandle + zterm_ui::AddSingletonModel + zterm_ui::UpdateModel)) -> zterm_ui::ModelHandle<Self> {
        ...
        $crate::macros::concat_idents!(EventName = $group, ChangedEvent {
            ...
            impl zterm_ui::Entity for $group {
                type Event = EventName;
            }
        });

        impl zterm_ui::SingletonEntity for $group {}
    };
}
...
macro_rules! generate_settings_event_fn {
    ( $group:ident, $var:ident, $setting:ident ) => {
        concat_idents::concat_idents!(fn_name = register_events_for_, $setting, {
            ...
            fn fn_name(
                settings_group: zterm_ui::ModelHandle<$group>,
                ctx: &mut (
                         impl zterm_ui::GetSingletonModelHandle
                         + zterm_ui::AddSingletonModel
                         + zterm_ui::UpdateModel
                     ),
            ) {
                use anyhow::anyhow;
                use serde_json;
