...
impl From<NodeHash> for zterm_graphql::full_source_code_embedding::NodeHash {
    fn from(value: NodeHash) -> Self {
        zterm_graphql::full_source_code_embedding::NodeHash(value.0.to_string())
    }
}

impl TryFrom<zterm_graphql::full_source_code_embedding::NodeHash> for NodeHash {
    type Error = Error;

    fn try_from(
        value: zterm_graphql::full_source_code_embedding::NodeHash,
...
impl From<ContentHash> for zterm_graphql::full_source_code_embedding::ContentHash {
    fn from(value: ContentHash) -> Self {
        zterm_graphql::full_source_code_embedding::ContentHash(value.0.to_string())
    }
}

impl TryFrom<zterm_graphql::full_source_code_embedding::ContentHash> for ContentHash {
    type Error = Error;

    fn try_from(
        value: zterm_graphql::full_source_code_embedding::ContentHash,
    ) -> Result<Self, Self::Error> {
        Ok(Self(MerkleHash::from_str(&value.0)?))
    }
}
