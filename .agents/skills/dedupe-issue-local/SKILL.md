...
description: Repo-specific dedupe guidance for Zterm. Only the categories declared overridable by the core dedupe-issue skill may be specialized here.
---

# Repo-specific dedupe guidance for `Zterm`
...
- Strip low-signal title prefixes such as `Bug:`, `Feature:`, `Request:`, `[Bug]`, `[Feature]`, `Zterm:`, and platform tags like `[macOS]`, `[Linux]`, or `[Windows]` before comparing titles.
...
- Do not collapse distinct Zterm surfaces just because they share a word like "agent", "terminal", "MCP", "settings", "search", or "sync". Require overlap in the actual failing behavior or requested capability.
