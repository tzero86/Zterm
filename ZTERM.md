...
### Build and Run
- `cargo run --bin zterm-oss --features gui` - Build and run Zterm locally (OSS build)
- `cargo run` - Same as above (default-run is set to `zterm-oss`)
- `cargo bundle --bin zterm-oss` - Bundle the main app

### Prerequisites (Windows)
- Install the Protocol Buffers compiler: `winget install Google.Protobuf`
  - `PROTOC` is pre-configured in `.cargo/config.toml` for the default WinGet install path.
  - If you install it elsewhere, update `.cargo/config.toml` accordingly.

### Running with local server
To connect Zterm client to a local server instance:

```bash
# Connect to server on default port 8080
cargo run --bin zterm-oss --features "gui,with_local_server"

# Connect to server on custom port (e.g., 8082)
SERVER_ROOT_URL=http://localhost:8082 WS_SERVER_URL=ws://localhost:8082/graphql/v2 cargo run --bin zterm-oss --features "gui,with_local_server"
```
...
- `cargo nextest run -p zterm_completer --features v2` - Run completer tests with v2 features
...
- `./script/run-clang-format.py -r --extensions 'c,h,cpp,m' ./crates/zterm_ui/src/ ./app/src/` - Format C/C++/Obj-C code
...
This is a Rust-based terminal emulator with a custom UI framework called **ZtermUI**.

### Key Components

**ZtermUI Framework** (`ui/`):
...
**Core Libraries**:
- `zterm_core/` - Core utilities and platform abstractions
...
5. **Cloud Sync**: Objects can be synchronized across devices via Zterm Drive
...
Zterm uses compile-time feature flags with a small runtime plumbing layer.

How to add a feature flag:
- Add a new variant to `zterm_core/src/features.rs` in the `FeatureFlag` enum
