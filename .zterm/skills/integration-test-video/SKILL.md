...
Use this skill when working with Zterm's integration test recording pipeline on this branch.
...
If you want the driver to auto-record a test or set of tests, add `ZTERM_INTEGRATION_TEST_VIDEO`:

```bash
WARPUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
ZTERM_INTEGRATION_TEST_VIDEO=test_video_recording \
cargo run -p integration --bin integration -- test_video_recording
```
...
```bash
WARPUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
ZTERM_INTEGRATION_TEST_VIDEO=test_foo,test_bar \
...
ZTERM_INTEGRATION_TEST_VIDEO=all
```
...
Important nuance:
- You do not need `ZTERM_INTEGRATION_TEST_VIDEO` if the test itself explicitly calls `with_start_recording()` and `with_stop_recording()`.
- Use the env var when you want whole-test recording without changing the test code.

### `ZTERM_INTEGRATION_TEST_ARTIFACTS_DIR`
...
### `ZTERM_INTEGRATION_TEST_VIDEO_DIR`
...
On this branch, the normal integration driver flow writes the finalized video into the test artifacts directory instead, so `ZTERM_INTEGRATION_TEST_ARTIFACTS_DIR` is the one you usually care about when reviewing results.
...
Set `ZTERM_INTEGRATION_TEST_VIDEO` to:
- `all`
- `1`
...
```text
${ZTERM_INTEGRATION_TEST_ARTIFACTS_DIR:-$TMPDIR/warp_integration_test_artifacts}/<test_name>/<timestamp>/
```
...
2. Decide whether recording should be explicit in the test or enabled via `ZTERM_INTEGRATION_TEST_VIDEO`.
