...
Linear: [APP-3575](https://linear.app/tzero86/issue/APP-3575/finalize-toml-schema-for-tab-configs)
...
2. Zterm writes the default template to `~/.zterm/tab_configs/my_tab_config.toml` and opens it in the user's configured editor.
...
- User invokes `/skills → tab-configs` or asks Zterm naturally (e.g. "create me a 2x2 tab config with one pane running my dev server").
- Zterm generates a valid `.toml` file and writes it to `~/.zterm/tab_configs/`.
...
- Zterm opens a new tab with the specified pane layout, running any configured commands.
...
- `worktree_name_autogenerated` (optional, bool, default false): when `true`, Zterm auto-generates the worktree branch name instead of prompting the user. The app detects worktree configs by scanning `commands` for `git worktree`.
...
8. The `tab-configs` bundled skill appears in `/skills` in Zterm.
