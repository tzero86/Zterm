...
if test -n "$ZTERM_INITIAL_WORKING_DIR"
    cd "$ZTERM_INITIAL_WORKING_DIR" >/dev/null 2>&1
    set -e ZTERM_INITIAL_WORKING_DIR
end

# Append additional PATH entries if provided via ZTERM_PATH_APPEND.
if test -n "$ZTERM_PATH_APPEND"
    set -gx --path PATH "$PATH:$ZTERM_PATH_APPEND"
    set -e ZTERM_PATH_APPEND
end
...
function warp_send_json_message
    ...
    if [ "$ZTERM_USING_WINDOWS_CON_PTY" = true ]
...
function  _warp_run_generator_command_internal
    ...
    fish -c "
        set -l warp_using_windows_con_pty $ZTERM_USING_WINDOWS_CON_PTY;
...
function warp_run_generator_command
    ...
    set -g _ZTERM_GENERATOR_COMMAND 1
...
function warp_update_prompt_vars
  ...
  # If honoring PS1, add prefix/suffix to both prompts
    ...
    function end_prompt        
      echo -n (printf '\x1b')
      if test "$ZTERM_HONOR_PS1" != "1" && [ "$ZTERM_USING_WINDOWS_CON_PTY" = true ]
...
# Changes the ZTERM_HONOR_PS1 variable to 1, to indicate we want to use the user's custom prompt. Restores
# the original fish prompt functions (which we set to empty for Zterm prompt) by calling warp_update_prompt_vars
...
# Changes the ZTERM_HONOR_PS1 variable to 0, to indicate we want to use the Warp prompt. Saves and clears
# the fish prompt functions (which we set to empty for Zterm prompt) by calling warp_update_prompt_vars
# to refresh the prompt. We force a repaint of the prompt to ensure the change is reflected immediately.
function warp_change_prompt_modes_to_warp_prompt
  set -x ZTERM_HONOR_PS1 "0"
...
function warp_precmd --on-event fish_prompt --on-event fish_posterror
    ...
    if ! test -z $_ZTERM_GENERATOR_COMMAND
        set -e _ZTERM_GENERATOR_COMMAND
    ...
    # We use the ESC-w bindkey for this ("w" for Zterm prompt).
    ...
    # in the case of Zterm prompt (for previewing the PS1). We send an escaped version of the raw prompt
...
if test "$ZTERM_IS_LOCAL_SHELL_SESSION" = "1"
    ...
    function warp_ssh_helper
        ...
        set -l zsh_env_script (printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; ZTERM_SESSION_ID="$(command -p date +%s)$RANDOM"; ZTERM_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; ZTERM_HONOR_PS1='$ZTERM_HONOR_PS1'; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $ZTERM_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d " \n"); printf '"'"'\x1b\x50\x24\x64%s\x9c'"'"' $_msg; unset _hostname _user _msg' | command od -An -v -tx1 | command tr -d ' \n')
...
hook="'$(printf "{\"hook\": \"SSH\", \"value\": {\"socket_path\": \"'$SSH_SOCKET_DIR/$ZTERM_SESSION_ID'\", \"remote_shell\": \"%s\"}}" "${SHELL##*/}" | command od -An -v -tx1 | command tr -d " \n")'"
...
bash)
    ...
    ;;
zsh) ZTERM_TMP_DIR="'$(mktemp -d warptmp.XXXXXX)'"
local ZSH_ENV_SCRIPT='$zsh_env_script'
if [[ "'$?'" == 0 ]]; then
  if command -v xxd >/dev/null 2>&1; then
    echo "'$ZSH_ENV_SCRIPT'" | command xxd -p -r > "'$ZTERM_TMP_DIR'"/.zshenv
  else
...
"
    ...
    function ssh
        if is_interactive_ssh_session $argv
            ...
            if [ "$ZTERM_USE_SSH_WRAPPER" = "1" ]
                if test $ZTERM_SHELL_DEBUG_MODE
                    set -g TRACE_FLAG_IF_ZTERM_SHELL_DEBUG_MODE "-x"
                else
                    set -g TRACE_FLAG_IF_ZTERM_SHELL_DEBUG_MODE ""
                end
