...
use crate::themes::theme::{ThemeKind, ZtermTheme, ZtermThemeConfig};
...
pub(super) fn parse_single_theme_dir_entry(item: &DirEntry) -> Option<(ThemeKind, ZtermTheme)> {
    parse_single_item_file(item, |file_name, mut theme: ZtermTheme| {
        ...
        let theme_kind = if let Some(name) = theme.name() {
            ZtermThemeConfig::file_to_theme(name, item.path().into())
        } else {
