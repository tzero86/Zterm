...
const PRIVACY_URL: &str = "https://github.com/tzero86/Zterm/blob/main/PRIVACY.md";
    ...
    A: Action + Clone,
{
    ...
    let text = "You are currently offline. An internet connection is required to use Zterm for the first time.";
    ...
    A: Action + Clone,
{
    ...
    let paragraph_2 = "However, we require users to be online when using Zterm for the first time in order to enable Zterm's AI and cloud features.";
    let paragraph_3 = "We offer cloud features to all users, and so we need an internet connection to meter AI usage, prevent abuse, and associate cloud objects with users. If you opt to use Zterm logged-out, a unique ID will be attached to an anonymous user account in order to support these features.";
...
pub fn render_privacy_settings_toggles<A: Action + Clone + 'static>(
    ...
    let telemetry_toggle = Flex::row()
        ...
        .with_child(
            Shrinkable::new(
                1.,
                render_privacy_settings_section_header("Help improve Zterm", appearance).finish(),
            )
            .finish(),
        )
        .with_child(
    ...
    let telemetry_description = render_description(
        appearance,
        "High-level feature usage data helps Zterm's product team prioritize the roadmap.".into(),
    );
    ...
    let crash_reporting_description = render_description(
        appearance,
        "Crash reporting helps Zterm's engineering team understand stability and improve performance.".into(),
    );
    ...
    let cloud_conversation_storage_description = render_description(
        ...
        if PrivacySettings::as_ref(app).is_cloud_conversation_storage_enabled {
            "Agent conversations can be shared with others and are retained when you log in on different devices. This data is only stored for product functionality, and Zterm will not use it for analytics."
        } else {
