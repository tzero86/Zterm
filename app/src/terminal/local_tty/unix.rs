...
fn build_host_shell_command(
    ...
    if let Some(version) = ChannelState::app_version() {
        ...
        builder.env("ZTERM_CLIENT_VERSION", version);
    } else {
        ...
        builder.env("ZTERM_CLIENT_VERSION", "local");
    }
    ...
    if enable_ssh_wrapper {
        builder.env("ZTERM_USE_SSH_WRAPPER", "1");
    } else {
        builder.env("ZTERM_USE_SSH_WRAPPER", "0");
    }
    ...
    builder.env("ZTERM_IS_LOCAL_SHELL_SESSION", "1");
    ...
    if FeatureFlag::HOANotifications.is_enabled() {
        builder.env(
            "ZTERM_CLI_AGENT_PROTOCOL_VERSION",
            current_protocol_version().to_string(),
        );
    }

    if shell_debug_mode {
        builder.env("ZTERM_SHELL_DEBUG_MODE", "1");
    }
    if honor_ps1 {
        builder.env("ZTERM_HONOR_PS1", "1");
    } else {
        builder.env("ZTERM_HONOR_PS1", "0");
    }
    ...
    let path_append = extra_path_entries()
        ...
        .join(":");
    builder.env("ZTERM_PATH_APPEND", path_append);
    ...
    if matches!(shell_starter.shell_type(), ShellType::Bash) {
        ...
        builder.env("ZTERM_INITIAL_HISTFILESIZE", sentinel_value);
    }
    ...
    if let Some(start_dir) = start_dir {
        builder.env("ZTERM_INITIAL_WORKING_DIR", start_dir);
    }
...
fn spawn_command_in_pty(
    ...
    #[cfg(target_os = "linux")]
    let is_isolated = zterm_isolation_platform::detect().is_some();
...
fn build_docker_sandbox_command(
    ...
    // split. Many of these (e.g. `ZTERM_USE_SSH_WRAPPER`,
    // `SSH_SOCKET_DIR`, `HISTFILESIZE`, `ZTERM_IS_LOCAL_SHELL_SESSION`)
    ...
    builder.env("ZTERM_HONOR_PS1", if honor_ps1 { "1" } else { "0" });
    let path_append = extra_path_entries()
        .map(|p| p.to_string_lossy().into_owned())
        .join(":");
    builder.env("ZTERM_PATH_APPEND", path_append);
    ...
    builder.env("HISTFILESIZE", sentinel_value);
    builder.env("ZTERM_INITIAL_HISTFILESIZE", sentinel_value);
    // Intentionally do NOT set `ZTERM_INITIAL_WORKING_DIR` for sandboxes:
...
/// Zterm panes/sandboxes don't race on or share the same host directories.
///
...
fn prepare_docker_sandbox(starter: &DockerSandboxShellStarter) -> Result<()> {
    ...
    // Zterm user. The file itself is left at the default mode so the
