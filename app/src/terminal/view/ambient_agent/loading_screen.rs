...
use zterm_ui::prelude::{CornerRadius, Radius};
...
pub fn render_cloud_mode_error_screen(
    ...
    let error_background = zterm_core::ui::color::coloru_with_opacity(error_color.into(), 10);
...
pub fn render_cloud_mode_github_auth_required_screen(
    ...
    let auth_icon = ConstrainedBox::new(Icon::Info.to_zterm_ui_icon(accent_color).finish())
        .with_width(ERROR_ICON_SIZE)
        .with_height(ERROR_ICON_SIZE)
        .finish();
    ...
    let auth_background: zterm_ui::elements::Fill = theme.surface_2().into();
...
pub fn render_cloud_mode_cancelled_screen(appearance: &Appearance) -> Box<dyn Element> {
    ...
    let cancelled_icon = ConstrainedBox::new(
        Icon::SlashCircle
            .to_zterm_ui_icon(title_color.into())
            .finish(),
    )
    .with_width(ERROR_ICON_SIZE)
    .with_height(ERROR_ICON_SIZE)
    ...
    // Dark background (surface_2) with subtle border
    let cancelled_background: zterm_ui::elements::Fill = theme.surface_2().into();
