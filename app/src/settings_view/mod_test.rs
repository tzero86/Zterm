...
fn ai_subpages_are_identified() {
    assert!(SettingsSection::ZtermAgent.is_ai_subpage());
...
fn code_subpages_are_identified() {
    ...
    assert!(!SettingsSection::ZtermAgent.is_code_subpage());
}
...
fn cloud_platform_subpages_are_identified() {
    ...
    assert!(!SettingsSection::ZtermAgent.is_cloud_platform_subpage());
}
...
fn ai_subpages_map_to_ai_backing_page() {
    assert_eq!(
        SettingsSection::ZtermAgent.parent_page_section(),
        SettingsSection::AI
    );
    assert_eq!(
...
fn ai_subpages_list_contains_all_ai_subpage_variants() {
    ...
    assert!(subpages.contains(&SettingsSection::ZtermAgent));
...
fn subpage_display_names_are_correct() {
    assert_eq!(SettingsSection::ZtermAgent.to_string(), "Zterm Agent");
...
fn subpage_from_str_parses_display_names() {
    // Both the legacy "Oz" name and the new "Zterm Agent" display name must
    // resolve to SettingsSection::ZtermAgent so existing deep links, persisted
...
fn search_knowledge_shows_only_knowledge_subpage() {
    ...
    filter.insert(SettingsSection::ZtermAgent, MatchData::Countable(0));
...
fn search_agent_shows_profiles_and_cli_agents() {
    ...
    assert!(!visible.contains(&SettingsSection::ZtermAgent));
...
fn nav_filter_includes_matching_subpage_and_excludes_others() {
    ...
    subpage_filter.insert(SettingsSection::ZtermAgent, MatchData::Countable(0));
...
fn collapsed_umbrella_is_a_single_nav_stop() {
    ...
    assert!(matches!(
        ...
        NavStop::CollapsedUmbrella {
            nav_index: 1,
            first_subpage: SettingsSection::ZtermAgent,
...
fn expanded_umbrella_produces_section_stop_per_subpage() {
    ...
    // Expect: Account, ZtermAgent, AgentProfiles, AgentMCPServers, Knowledge,
...
fn collapsed_umbrella_with_filtered_subpages_uses_first_visible_subpage() {
    ...
    let stops = build_nav_stops(&nav_items, |section| {
        // Hide ZtermAgent (first AI subpage); keep the rest.
        section != SettingsSection::ZtermAgent
    });
    ...
    match agents_stop {
        NavStop::CollapsedUmbrella {
            ...
            assert_eq!(
                *first_subpage,
                SettingsSection::AgentProfiles,
                "ZtermAgent is hidden by the filter, so the first visible subpage is AgentProfiles"
            );
            assert_eq!(
                *last_subpage,
                SettingsSection::ThirdPartyCLIAgents,
                "last_subpage is unaffected by hiding ZtermAgent and should remain the last visible subpage"
            );
        }
        _ => unreachable!(),
    }
...
fn arrow_down_from_account_with_collapsed_agents_lands_on_first_subpage() {
    ...
    // Pressing Down from Account should auto-expand Agents and select ZtermAgent,
    ...
    assert_eq!(next, SettingsSection::ZtermAgent);
}
...
fn arrow_up_into_collapsed_umbrella_respects_search_filter_for_last_subpage() {
    ...
    // or on the first subpage ZtermAgent.
    let next = simulate_cycle(
        &nav_items,
        &stops,
        SettingsSection::BillingAndUsage,
...
fn arrow_down_collapsed_umbrella_respects_search_filter() {
    let nav_items = realistic_nav_items();
    // Search filter hides ZtermAgent and AgentProfiles so the first visible AI
    // subpage is AgentMCPServers.
    let is_visible = |section: SettingsSection| {
        !matches!(
            section,
            SettingsSection::ZtermAgent | SettingsSection::AgentProfiles
        )
    };
    let stops = build_nav_stops(&nav_items, is_visible);

    // From Account, Down should land on AgentMCPServers (first visible
    // subpage of the still-collapsed Agents umbrella), not on ZtermAgent /
    // AgentProfiles.
    let next = simulate_cycle(
        &nav_items,
        &stops,
