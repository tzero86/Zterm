...
use zterm_completer::completer::PathCompletionContext;
use zterm_completer::completer::{CompletionContext, EngineDirEntry};
        ...
        VirtualFS::test(
            ...
            |dirs, mut sandbox| {
                ...
                sandbox.touch(vec![
                    ...
                    Stub::EmptyFile("target/debug/zterm_ui"),
                ]);
                ...
                assert_eq!(
                    HashSet::<EngineDirEntry>::from_iter(Arc::unwrap_or_clone(
                        zterm_ui::r#async::block_on(ctx.list_directory_entries(tests_dir))
                    )),
                    HashSet::from_iter([
        ...
        VirtualFS::test(
            ...
            |dirs, mut sandbox| {
                ...
                let mut entries = HashSet::<EngineDirEntry>::from_iter(Arc::unwrap_or_clone(
                    zterm_ui::r#async::block_on(ctx.list_directory_entries(cwd)),
                ));
    ...
    App::test((), |app| async move {
        VirtualFS::test(
            ...
            |dirs, mut sandbox| {
                ...
                let mut entries = HashSet::<EngineDirEntry>::from_iter(Arc::unwrap_or_clone(
                    zterm_ui::r#async::block_on(ctx.list_directory_entries(test_dir)),
                ));
                // TODO(CORE-2000): The ls script we use to list entries in remote
                // sessions adds a spurious "." directory when run in the VirtualFS.
                // As a temporary workaround, we remove this file in the test.
