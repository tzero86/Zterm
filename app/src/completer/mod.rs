...
impl CompletionContext for SessionContext {
    ...
    #[cfg(feature = "completions_v2")]
    fn js_context(&self) -> Option<&dyn zterm_completer::completer::JsExecutionContext> {
        self.js_ctx
            .as_ref()
            .map(|ctx| -> &dyn zterm_completer::completer::JsExecutionContext { ctx })
    }
...
fn ls_script_for_dir(directory: &TypedPath) -> Option<String> {
    ...
    let escaped_dir = zterm_util::path::ShellFamily::Posix.shell_escape(dir_str);
