...
use zterm_ui::{platform::WindowStyle, App, ModelHandle};
...
fn std_path(path: &std::path::Path) -> zterm_util::standardized_path::StandardizedPath {
    zterm_util::standardized_path::StandardizedPath::try_from_local(path).unwrap()
}
...
fn repo_transition_unregisters_lazy_loaded_path() {
    VirtualFS::test("file_tree_repo_transition", |dirs, mut vfs| {
        ...
        let canonical_repo_root =
            zterm_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo_root)
                .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &zterm_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &zterm_util::standardized_path::StandardizedPath::try_from_local(&src_dir)
                        .unwrap(),
                    ctx,
                );
            });
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &zterm_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &zterm_util::standardized_path::StandardizedPath::try_from_local(&nested_dir)
                        .unwrap(),
                    ctx,
                );
            });
