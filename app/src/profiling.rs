...
//! [`zterm_core::paths::state_dir()`].  Otherwise, profiles will be written
...
fn pprof_binary_path() -> anyhow::Result<std::path::PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            let app_bundle_dir = std::path::PathBuf::from(zterm_core::macos::get_bundle_path().context("Failed to get app bundle path")?);
...
fn profile_output_dir() -> std::path::PathBuf {
    cfg_if::cfg_if! {
        if #[cfg(feature = "release_bundle")] {
            zterm_core::paths::secure_state_dir().unwrap_or(zterm_core::paths::state_dir())
        } else {
