...
lazy_static! {
    ...
    static ref ZTERM_SECRET_DELIMETER: &'static str = "/warp-secret-delimeter/";
    static ref LASTPASS_LIST_SECRETS_COMMAND: Vec<String> = {
        vec![
            "lpass".to_owned(),
            "ls".to_owned(),
            format!("--format=%an{}%ai", *ZTERM_SECRET_DELIMETER),
        ]
    };
}
...
fn parse_lastpass_secrets(output: &str) -> anyhow::Result<Vec<ExternalSecret>> {
    let parsed_output: Vec<ExternalSecret> = output
        .lines()
        .filter_map(|line| {
            let parts = line.split(*ZTERM_SECRET_DELIMETER).collect_vec();
