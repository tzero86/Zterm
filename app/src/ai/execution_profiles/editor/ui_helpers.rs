...
pub fn render_models_section(
    ...
    let mut column = Flex::column()
        ...
        .with_child(render_filterable_dropdown_row(
            appearance,
            "Base model",
            "This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Zterm may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.",
