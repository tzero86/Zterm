...
impl PaneContent for WelcomePane {
    ...
    fn detach(
        ...
        ctx: &mut zterm_ui::ViewContext<PaneGroup>,
    ) {
        let child = self.view.as_ref(ctx).child(ctx);
    ...
    fn has_application_focus(&self, ctx: &mut zterm_ui::ViewContext<PaneGroup>) -> bool {
        self.view.is_self_or_child_focused(ctx)
    }

    fn focus(&self, ctx: &mut zterm_ui::ViewContext<PaneGroup>) {
        self.view
            .as_ref(ctx)
            .child(ctx)
