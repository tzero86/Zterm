...
/// Keep in sync with [`zterm_cli::AppArgs`].
...
pub fn is_crash_recovery_process(args: &zterm_cli::AppArgs) -> bool {
...
impl CrashRecoveryProcess {
    ...
    fn kill(&mut self) {
        ...
        *IS_CRASH_RECOVERY_PROCESS_RUNNING.write() = false;
        self.is_alive = false;
        zterm_logging::on_crash_recovery_process_killed();
    }
...
impl CrashRecovery {
    ...
    #[cfg(test)]
    pub fn register_for_test(app: &mut zterm_ui::App) {
        ...
        app.update(|ctx| {
            ctx.add_singleton_model(|ctx| {
                ...
                let launch_mode = crate::LaunchMode::App {
                    args: zterm_cli::AppArgs::default(),
                    api_key: None,
                };
...
#[cfg(windows)]
fn wait_for_parent_crash(args: &zterm_cli::AppArgs) {
    use windows::Win32::{
...
fn handle_parent_crash(
    ...
    zterm_logging::on_parent_process_crash();
